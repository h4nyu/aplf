...
import pandas as pd
from .model import UNet, TinyUNet
...
def boost_fit(model_id,
    ...
    for e in range(epochs):
        for train_sample, val_sample in zip(train_loader, val_loader):
            ...
            output = None
            optimizer.zero_grad()
            output = models[0](train_image)
            for m in models[1:]:
                output = m(output, train_image)
            loss = critertion(
            ...
            output = models[0](val_image)
            for m in models[1:]:
                output = m(output, val_image)
            val_loss = critertion(
                F.log_softmax(output, dim=1),
                val_mask
            )
